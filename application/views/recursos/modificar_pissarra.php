...
<div class="card mx-auto" style="width: 50%; text-align: center;">
    <div class="card-body">
        <form enctype="multipart/form-data" method="POST" action="<?php echo base_url('recursos/modificar_pissarra'); ?>">
            <input name="id" value="<?php echo $recursos['id']; ?>">
            ...
            <script>
                ...
                // canvas.loadImatge("<?php echo $recursos['pissarra'] ?>");
                var img = new Image();
                img.src = "data:image/png;base64,<?php echo $recursos['pissarra'] ?>";
                img.onload = function() {
                    canvas.setImage(img);
                };
                ...
                <tbody>
                    <?php foreach ($adjunts as $adjunts_item) : ?>
                        <tr>
                            ...
                            <td><a class="btn btn-outline-danger" href="<?php echo base_url('recurs/pissarra/borrar/arxius/' . $id_recurs  . '/adjunts/' . $adjunts_item['id']); ?>"><i class="fas fa-times"></i>&nbsp;Borrar</a></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
            ...
            <button type="submit" onclick="saveCanvas()" class="btn btn-warning" name="submit"><i class="far fa-edit"></i>&nbsp;MODIFICAR</button>
    </div>
    </form>
    <?php
